...
No client library is needed and that is the test the design was written
against:
...
JSON-RPC 2.0, one compact object per line, over a Unix socket. `serde_json`'s
writer never emits a bare newline, so a value cannot break the framing. Batches
(§6) are refused with a reason rather than silently: every method here changes
the terminal or waits on it, nothing wants one, and a client with two things to
do sends two lines.
...
**The directory is the access control**: `0700`, the socket `0600`, and
`SO_PEERCRED` behind both. Anything that reaches this socket can type at
whatever the window is connected to.
