...
| 21 | Per-terminal counters: bytes, rates, connect time, and a serial port's live control lines | Nothing counts anything; the control lines are visible only to a macro's `getmodemstatus` | 0.5.5 |
...
## 21. Counters, rates, connect time and serial state
